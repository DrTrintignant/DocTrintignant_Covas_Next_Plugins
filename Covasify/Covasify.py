...
class COVASIFYPlugin(PluginBase):
    ...
    def initialize_spotify(self, credentials: dict):
        ...
        try:
            ...
            log('info', f'COVASIFY: OAuth cache path: {cache_path}')
            ...
            log('error', f'COVASIFY: Failed to initialize Spotify: {self.last_init_error}')
            self.sp = None
            return False
        ...
        try:
            ...
            if self.sp:
                ...
                if self.last_init_error:
                    return f"COVASIFY Test: {name} v{version} - Active but not connected. Spotify initialization error: {self.last_init_error}"
                return f"COVASIFY Test: {name} v{version} - Active but not connected. No Spotify initialization error was captured. Check credentials at: {plugin_folder}"
